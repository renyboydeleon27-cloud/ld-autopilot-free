(()=>{
  ...
  function buildPrompt(topic,format,existing){
    ...
    const layout=format==='longform'
      ...
      : 'APPROVED PORTRAIT CROP-SAFE LAYOUT: deliver portrait 9:16 with essential content inside the central 72% of canvas height (14%-86%) and at least 6% side margins. Keep the top 14% and bottom 14% as storm/scene background bleed only, with no essential lettering or anatomy. This preserves the composition in a centered 3:4 crop. Compact title block at 15%-33% height. Raise the adult survivor: head near 38%-43%, torso and BOTH hands fully visible by 72%; modestly reduce subject scale when necessary. Reduce empty sky between headline and face. '+(showDeathBadge?'Place the verified-death curiosity badge at right around 35%-49%. ':'Do NOT place any casualty/death badge; use that area for disaster visuals and breathing room. ')+'Keep the entire footer at 78%-85%. No cropped letters, face, hands, logo or footer; no text over the face. Check both the full portrait and centered 3:4 crop before finishing. Arbitrary extra zoom must not be assumed safe.';
  ...
  function apply(){
    ...
    prompt.value=next;
    prompt.dataset.thumbnailFormatLock='v3.35.8-verified-death-badge';
    fire(prompt);
    return true;
  }
