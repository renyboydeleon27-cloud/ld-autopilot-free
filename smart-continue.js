...
function currentApiUsage(){
  ...
  if(!u||typeof u!=='object'||Array.isArray(u)||u.topic!==topic()||u.format!==format()){
    window.ldApiUsage=emptyApiUsage();
  }
...
function mount(){
  ...
  if(!sticky){
    ...
    sticky.innerHTML='<div class="ld-smart-target">CURRENT TARGET: —</div><div class="ld-api-cost-counter">PROJECT API · 0 calls · $0.00000 est.</div><button type="button" id="ldSmartStickyBtn" class="primary">🚀 SMART CONTINUE</button><div id="ldSmartStickyStatus" class="ld-smart-sticky-status">Ready.</div>';
...
function refreshApiCostCounter(){
  const u=currentApiUsage();
  const text='PROJECT API · '+Math.round(usageNumber(u.calls))+' call'+(Math.round(usageNumber(u.calls))===1?'':'s')+' · '+formatTokenCount(u.totalTokens)+' tokens · '+formatApiCost(u.estimatedCostUsd)+' est.';
  document.querySelectorAll('.ld-api-cost-counter').forEach(el=>{
    el.textContent=text;
    el.title='Estimated tracked OpenAI API cost for this project from API-reported token usage. This is not your OpenAI account balance. Pricing snapshot: '+(u.priceSnapshot||'unknown')+'.';
  });
