...
function normalizedSignature(value){try{var parts=JSON.parse(value);parts[4]=clean(parts[4]);parts[5]=clean(parts[5]);return JSON.stringify(parts);}catch(e){return '';}}
function pinScene(card){if(!state(card).scene)card.dataset.videoScene=clean(sceneFrom(card));}
function signature(card){return JSON.stringify([current(),format(),style(),continuity(),clean(sceneFrom(card)),clean(card.querySelector('.narration').value)]);}
...
if(mode==='text'&&!state(card).text&&ready()){try{pinScene(card);card.dataset.textVideoPrompt=build(card);card.dataset.textVideoSignature=signature(card);var text=card.querySelector('.text-video-prompt');if(text)text.value=state(card).text;built++;}catch(e){/* Keep the panel's missing-prompt status visible. */}}
...
box.querySelector('.build-missing-video').onclick=function(){if(!ready())return showToast('Fill in the shared year and location first.');document.querySelectorAll('.stage-card').forEach(function(card){if(supports(card)&&!state(card).text)generate(card);});syncGlobalControl();showToast('Missing panel prompts built. Use the Video method buttons above to switch all panels.');};
