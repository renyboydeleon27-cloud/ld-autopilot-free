(()=>{
  ...
  function eraForYear(year){
    ...
    if(year<=1929)return {label:`${year} · STRICT B/W early-film/newsreel`,look:'STRICT MONOCHROME LOCK: render the ENTIRE shot in true black-and-white grayscale from the first frame through the final frame. ZERO color, ZERO selective color, ZERO sepia, ZERO tinting, ZERO colorization, and no color returning during flashes, fire, lightning, sky, clothing, skin, water, vegetation, or debris. Treat this as a historical live-action recreation captured with an early-film/newsreel aesthetic: soft optical detail, organic film grain, slight gate weave, restrained exposure flicker, modest contrast and period-plausible motion cadence. Avoid VHS, modern HD sharpness and modern digital grading. BLACK-AND-WHITE continuity has higher priority than any generic natural-color instruction.'};
